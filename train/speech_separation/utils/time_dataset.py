...
class TimeDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        inputs, s1 = self.processer.process(
...
def _dochunk(wav_list, index, segement_length, sample_rate, num_threads=12):
    ...
    if stride < 100:
        p = mp.Process(
            target=worker,
            args=(wav_list, index, 0, len(wav_list), segement_length, sample_rate),
        )
        p.start()
        pc_list.append(p)
    else:
        for idx in range(num_threads):
            ...
            p = mp.Process(
                target=worker,
                args=(wav_list, index, idx * stride, end, segement_length, sample_rate),
            )
            p.start()
            pc_list.append(p)
    for p in pc_list:
