...
from utils.misc import EPS, power_compress, stft
...
class Solver(object):
    ...
    def _load_model(self, mode="last_checkpoint", use_cuda=True, strict=True):
        ...
        if not os.path.isfile(ckpt_name):
            ...
            if "model" in checkpoint:
                ...
                print(
                    "Checkpoint {ckpt_name} has no model key, will try _load_pretrained_model"
                )
                return 0
            try:
                ...
                if (
                    ...
                    and self.args.network == "MossFormer2_SE_48K"
                ):
                    print(
                ...
                if (
                    ...
                    and self.args.network == "MossFormerGAN_SE_16K"
                ):
                    print(
            ...
            if self.discriminator is not None and disc_name is not None:
                ...
                self.discriminator.load_state_dict(
                    checkpoint["discriminator"], strict=strict
                )
    ...
    def _load_pretrained_model(
        self, checkpoint_path, load_optimizer=False, load_training_stat=False
    ):
        if os.path.isfile(checkpoint_path):
            ...
            if self.discriminator is not None:
                ...
                self.optimizer_disc.load_state_dict(checkpoint["optimizer_disc"])
            print(
                f"==> Done model init from {checkpoint_path}. Start finetune training ..."
            )
        else:
    ...
    def save_checkpoint(self, mode="last_checkpoint"):
        checkpoint_path = os.path.join(
            ...
            "model.ckpt-{}-{}.pt".format(self.epoch, self.step),
        )
        torch.save(
            {
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.args.max_epoch + 1):
            ...
            if self.args.distributed:
                tr_loss = self._reduce_tensor(tr_loss.to(self.device))
            if self.print:
                print(
            ...
            with torch.no_grad():
                ...
                if self.args.distributed:
                    val_loss = self._reduce_tensor(val_loss.to(self.device))
            if self.print:
                print(
            ...
            if self.args.tt_list is not None:
                ...
                start = time.time()
                with torch.no_grad():
                    test_loss = self._run_one_epoch(
                        data_loader=self.test_data, state="test"
                    )
                    if self.args.distributed:
                        test_loss = self._reduce_tensor(test_loss.to(self.device))
                if self.print:
                    print(
    ...
    def _run_one_epoch_mossformergan_se_16k(self, data_loader, state="train"):
        ...
        for i, (inputs, labels) in enumerate(data_loader):
            ...
            loss_gen, loss_disc = loss_mossformergan_se_16k(
                self.args, inputs, labels, Out_List, c, self.discriminator, self.device
            )
            ...
            if state == "train":
                if self.args.accu_grad:
                    if loss_gen is not None:
                        self.accu_count += 1
                        loss_gen_scaled = loss_gen / (
                            self.args.effec_batch_size / self.args.batch_size
                        )
                        loss_gen_scaled.backward()
                        torch.nn.utils.clip_grad_norm_(
                            self.model.parameters(), self.args.clip_grad_norm
                        )
                        loss_disc_scaled = loss_disc / (
                            self.args.effec_batch_size / self.args.batch_size
                        )
                        loss_disc_scaled.backward()
                        torch.nn.utils.clip_grad_norm_(
                            self.discriminator.parameters(), self.args.clip_grad_norm
                        )
                        if self.accu_count == (
                            self.args.effec_batch_size / self.args.batch_size
                        ):
                ...
                if (i + 1) % self.args.print_freq == 0:
                    ...
                    print(
                        "Train Epoch: {}/{} Step: {}/{} | {:2.3f}s/batch | lr {:1.4e} |"
                        "| Gen_Loss {:2.4f}"
                        "| Disc_Loss {:2.4f}".format(
                            self.epoch,
                            self.args.max_epoch,
                            i + 1,
                            num_batch,
    ...
    def _run_one_epoch_frcrn_se_16k(self, data_loader, state="train"):
        ...
        for i, (inputs, labels) in enumerate(data_loader):
            ...
            loss, CMask_MSE_Loss, SiSNR_loss = loss_frcrn_se_16k(
                self.args, inputs, labels, Out_List, self.device
            )
