    ...
    def _load_model(self, path, load_optimizer=False, load_training_stat=False):
        ...
        for key in state.keys():
            if (
                ...
                f"module.{key}" in pretrained_model
                and state[key].shape == pretrained_model[f"module.{key}"].shape
            ):
        ...
        for epoch in range(self.start_epoch, self.args.max_epoch + 1):
            ...
            if self.print:
                print(
                    ...
                    "Train Loss {2:.3f}".format(epoch, time.time() - start, tr_loss)
                )
            ...
            if self.print:
                print(
                    ...
                    "Valid Loss {2:.3f}".format(epoch, time.time() - start, val_loss)
                )
            ...
            if self.print:
                print(
                    ...
                    "Test Loss {2:.3f}".format(epoch, time.time() - start, test_loss)
                )
            ...
            if self.halving:
                self.halving = False
                self._load_model(
                    f"{self.args.checkpoint_dir}/last_best_checkpoint.pt",
                    load_optimizer=True,
                )
                if self.print:
                    print("reload weights and optimizer from last best checkpoint")
            ...
            if self.print:
                ...
                if find_best_model:
                    torch.save(
                        checkpoint,
                        self.args.checkpoint_dir + "/last_best_checkpoint.pt",
                    )
    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():
            for i, (a_mix, a_tgt, ref_tgt) in enumerate(data_loader):
                ...
                pesqi = pesq(self.args.audio_sr, a_tgt, a_tgt_est, "wb") - pesq(
                    self.args.audio_sr, a_tgt, a_mix, "wb"
                )
                avg_pesqi += pesqi

                stoii = stoi(
                    a_tgt, a_tgt_est, self.args.audio_sr, extended=False
                ) - stoi(a_tgt, a_mix, self.args.audio_sr, extended=False)
                avg_stoii += stoii
