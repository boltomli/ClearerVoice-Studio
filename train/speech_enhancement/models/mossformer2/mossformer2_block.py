...
from models.mossformer2.conv_module import ConvModule, FFConvM_Dilated
...
class FFConvM_Small(nn.Module):
    def __init__(
        self, dim_in, dim_out, norm_klass=nn.LayerNorm, dropout=0.1, reduction=4
    ):
        super().__init__()
        self.mdl = nn.Sequential(
            norm_klass(dim_in),
...
class FLASH_ShareA_FFConvM_Dilated(nn.Module):
    def __init__(
        self,
        *,
        dim,
...
class FLASH_ShareA_FFConvM_A256(nn.Module):
    def __init__(
        self,
        *,
        dim,
...
class Gated_FSMN_Dilated_Dual(nn.Module):
    def __init__(self, in_channels, out_channels, lorder, hidden_size):
        super().__init__()
        self.to_u = FFConvM_Dilated(
            dim_in=in_channels,
            dim_out=hidden_size,
            norm_klass=nn.LayerNorm,
            dropout=0.1,
        )
        self.to_v = FFConvM_Dilated(
            dim_in=in_channels,
            dim_out=hidden_size,
            norm_klass=nn.LayerNorm,
            dropout=0.1,
...
class Gated_FSMN_Block_Dilated_Dual(nn.Module):
    """1-D convolutional block."""
    ...
    def __init__(
        ...
        super(Gated_FSMN_Block_Dilated_Dual, self).__init__()
        ...
        self.norm1 = CLayerNorm(inner_channels)
        self.gated_fsmn = Gated_FSMN_Dilated_Dual(
            inner_channels, inner_channels, lorder=20, hidden_size=inner_channels
        )
        self.norm2 = CLayerNorm(inner_channels)
        self.conv2 = nn.Conv1d(inner_channels, dim, kernel_size=1)
