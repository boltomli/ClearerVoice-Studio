...
from utils.misc import batch_pesq, istft, power_compress, power_uncompress, stft
...
def cal_SISNR(source, estimate_source):
    ...
    proj = (
        torch.sum(source * estimate_source, axis=-1, keepdim=True) * source / ref_energy
    )
    # e_noise = s' - s_target
...
def psm_loss(noisy_stft, clean_stft, mask_list, device, ratio_mod="l2"):
    ...
    loss = (
        0.5
        * torch.sum(torch.pow(pred_mask - tgt_mask, 2) * noisy_abs_norm)
        / total_frames
    )
...
def loss_complex_mask(args, noisy_wav, clean_wav, est_cmask, device):
    ...
    torch.sqrt(Y_pow)
    gth_mask = torch.cat(
        [(Sr * Yr + Si * Yi) / (Y_pow + 1e-8), (Si * Yr - Sr * Yi) / (Y_pow + 1e-8)], 1
    )
    gth_mask[gth_mask > 2] = 1
    gth_mask[gth_mask < -2] = -1
    mask_real_loss = (
        F.mse_loss(gth_mask[:, :fft_bins, :], est_cmask[:, :fft_bins, :]) * args.fft_len
    )
    mask_imag_loss = (
        F.mse_loss(gth_mask[:, fft_bins:, :], est_cmask[:, fft_bins:, :]) * args.fft_len
    )
