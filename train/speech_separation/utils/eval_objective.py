...
def main(args):
    pathe = (
        args.pathe
    )  #'/home/work_nfs3/yxhu/workspace/se-cldnn-torch/exp/cldnn_2_1_1_0.0005_16k_6_9/rec_wav/'
    ...
    with open(args.result_list, "w") as wfid:
        ...
        for eval_score in results:
            ...
            stoi = score["stoi"]
            si_sdr = score["si_sdr"]
            wfid.writelines("{:s},{:.3f},{:.3f}, ".format(utt_id, pesq[0], pesq[1]))
            wfid.writelines("{:.3f},{:.3f}, ".format(stoi[0], stoi[1]))
            wfid.writelines("{:.3f},{:.3f}\n ".format(si_sdr[0], si_sdr[1]))


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--wav_list", type=str, default="wav.lst")

    parser.add_argument("--result_list", type=str, default="result_list")

    parser.add_argument("--num_threads", type=int, default=24)
    parser.add_argument("--pathe", type=str, default="./rec")
    parser.add_argument(
        "--pathc",
        type=str,
