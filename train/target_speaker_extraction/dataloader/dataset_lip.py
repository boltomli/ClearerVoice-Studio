...
class dataset_lip(data.Dataset):
    def __init__(self, args, partition):
        ...
        mix_lst = list(
            filter(lambda x: x.split(",")[0] == partition, mix_lst)
        )  # [:200]
        mix_lst = sorted(
    ...
    def _load_raw_video(self, visual_path, min_length_visual):
        ...
        if visual.shape[0] < min_length_visual:
            visual = np.pad(
                visual,
                ((0, int(min_length_visual - visual.shape[0])), (0, 0), (0, 0)),
                mode="constant",
            )
        return visual
    ...
    def _audioread(self, path, min_length_audio, sampling_rate):
        ...
        if data.shape[0] < min_length_audio:
            data = np.pad(
                data, (0, int(min_length_audio - data.shape[0])), mode="constant"
            )
        return data
    ...
    def __getitem__(self, index):
        ...
        min_length_second = float(
            batch_lst[-1].split(",")[-1]
        )  # truncate to the shortest utterance in the batch
        ...
        for line_cache in batch_lst:
            ...
            if self.args.speaker_no == 2:
                ...
                a_int2 = self._audioread(
                    int_audio_path_2, min_length_audio, self.audio_sr
                )
        ...
        return (
            ...
            np.asarray(tgt_visuals, dtype=np.float32),
        )
