...
def main(args):
    ...
    if args.network == "FRCRN_SE_16K":
        ...
        optimizer_disc = torch.optim.AdamW(
            discriminator.parameters(), lr=args.init_learning_rate * 2
        )
    else:
...
if __name__ == "__main__":
    ...
    # experiment setting
    parser.add_argument(
        "--mode", type=str, default="train", help="run train or inference"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--print_freq", type=int, default=10, help="No. steps waited for printing info"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cv-list", dest="cv_list", type=str, help="the cross-validation data list"
    )
    parser.add_argument(
        "--tt-list",
        dest="tt_list",
    ...
    parser.add_argument(
        "--max_length", type=int, help="max_length of mixture in training"
    )
    parser.add_argument(
        "--num_workers", type=int, help="Number of workers to generate minibatch"
    )
    parser.add_argument(
        "--sampling-rate", dest="sampling_rate", type=int, default=16000
    )
    parser.add_argument(
        "--load_fbank",
        type=int,
        default=None,
        help="calculate and load fbanks for inputs",
    )
    # FFT
    parser.add_argument(
        "--window-len", dest="win_len", type=int, help="the window-len in enframe"
    )
    parser.add_argument(
        "--window-inc",
        dest="win_inc",
    ...
    parser.add_argument(
        "--max-epoch", dest="max_epoch", type=int, default=20, help="the max epochs"
    )
    parser.add_argument(
        "--num-gpu", dest="num_gpu", type=int, default=1, help="the num gpus to use"
    )
    parser.add_argument("--init_learning_rate", type=float, help="Init learning rate")
    parser.add_argument(
        "--finetune_learning_rate", type=float, help="Finetune learning rate"
    )
    parser.add_argument(
        "--weight-decay", dest="weight_decay", type=float, default=0.00001
    )
    parser.add_argument(
        "--clip-grad-norm", dest="clip_grad_norm", type=float, default=10.0
    )
