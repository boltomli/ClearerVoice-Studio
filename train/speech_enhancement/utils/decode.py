...
import torch
import torchaudio
from utils.misc import compute_fbank, istft, power_compress, power_uncompress, stft
...
def decode_one_audio_frcrn_se_16k(model, device, inputs, args):
    ...
    )  # decoding window length 16000 # 1s
...
def decode_one_audio_mossformer2_se_48k(model, device, inputs, args):
    ...
    if (
        input_len > args.sampling_rate * args.one_time_decode_length
    ):  ## longer than 20s, use online decoding
        online_decoding = True
        if online_decoding:
            window = int(
                args.sampling_rate * args.decode_window
            )  ## 4s window for 48kHz sample rate
