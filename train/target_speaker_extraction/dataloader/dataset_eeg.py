...
class dataset_eeg(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for line_cache in batch_lst:
            ...
            a_tgt, _ = sf.read(
                tgt_audio_path, start=int(start), stop=int(end), dtype="float32"
            )
            ...
            a_int, _ = sf.read(
                int_audio_path, start=int(start), stop=int(end), dtype="float32"
            )
        ...
        return (
            ...
            np.asarray(tgt_eegs, dtype=np.float32),
        )
